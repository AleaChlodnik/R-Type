    ...
    EntityInformation desc, ComponentManager componentManager, EntityManager entityManager)
{
    ...
    descLeft = ((desc.vPos.x / 100) * SCREEN_WIDTH) -
        ((desc.animationComponent.dimension.x * desc.spriteData.scale.x) / 2);
    descRight = ((desc.vPos.x / 100) * SCREEN_WIDTH) +
        ((desc.animationComponent.dimension.x * desc.spriteData.scale.x) / 2);
    descTop = ((desc.vPos.y / 100) * SCREEN_HEIGHT) -
        ((desc.animationComponent.dimension.y * desc.spriteData.scale.y) / 2);
    descBottom = ((desc.vPos.y / 100) * SCREEN_HEIGHT) +
        ((desc.animationComponent.dimension.y * desc.spriteData.scale.y) / 2);
