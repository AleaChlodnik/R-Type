...
class Rtype {
  ...
  public:
    Rtype(); // This will init the background, and player. Default easy mode
             // and normal daltonism mode. Ex: renderSystem.addEntity(player),
             // inputSystem.addEntity(player),
             // collisionSystem.addEntity(player), etc.
    void run(); // If main_menu variable is true, call mainMenu. While
                // main_menu is false, call gameLoop. Get messages from the
                // server, and store them in a queue (maybe?).

    void
    mainMenu(); // Open window. (handleEvents). Display the main menu with
                ...
                // color filter over the screen until deactivated.
    void
    gameLoop(); // Open window. This is where I will call the handleEvents,
                // updateGame, processCommands, and render functions.
    void handleEvents(); // This is where I will handle the events for the
                         // window & player (key input, etc.).
    void
    updateGame(); // This is where I will update the time, position of sprites,
                  // etc. Ex: inputSystem.update(deltaTime.asSeconds()),
                  // renderSystem.update(deltaTime.asSeconds()), etc.
    void processServerMessages(); // This is where I will process the commands
                                  // from the server. I will check the queue,
                                  // call the function, then pop from queue.
    void renderGame(); // Ex: window.clear(), window.draw(background),
